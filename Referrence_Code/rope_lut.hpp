...
const int SIN_COS_LUT_SIZE = 1024;  // 2^10 cho quarter-wave [0, π/2)
const int INV_FREQ_LUT_SIZE = 384;
const int UNROLL_FACTOR = 8;

const data_t bien = data_t(651.8986469044033); // 1/((pi/2)/1024) = 1024/(pi/2)
...
void rotator(data_t q_or_k[DIM], data_t rotated_pairs[HALF_DIM][2]) {
    #pragma HLS INLINE
...
static void sincos_producer(token_t token_id, hls::stream<SinCosPair> &out_stream) {
    ...
    PARALLEL_PRODUCER: for (int j = 0; j < 4; j++) {
        ...
        PRODUCER_LOOP: for (int i = 0; i < 96; ++i) {
            ...
            addr_t addr = address_generator(i + j * 96);
            SinCosPair p;
