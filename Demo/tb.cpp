...
// Prototype for your HLS function
extern "C" {
void llama_layer(
    float* token_embed,
    float* output_logits,
...
// Global variables moved outside main
...
std::cout << "DEBUG: config read: dim="<<config.dim
          << " hidden_dim="<<config.hidden_dim
          << " n_layers="<<config.n_layers
          << " n_heads="<<config.n_heads
          << " n_kv_heads="<<config.n_kv_heads
          << " vocab_size="<<config.vocab_size
          << " seq_len="<<config.seq_len << std::endl;// ...existing code...
    int shared_weights = config.vocab_size > 0 ? 1 : 0;
    config.vocab_size = std::abs(config.vocab_size);

    // Calculate total weights size (CORRECTED based on llama2 architecture)
    size_t head_size = config.dim / config.n_heads;
    size_t total_weights = 0;
    total_weights += (size_t)config.vocab_size * config.dim;  // token_embedding_table
    total_weights += (size_t)config.n_layers * config.dim;    // rms_att_weight
    total_weights += (size_t)config.n_layers * config.dim * (config.n_heads * head_size);  // wq
    total_weights += (size_t)config.n_layers * config.dim * (config.n_kv_heads * head_size); // wk
    total_weights += (size_t)config.n_layers * config.dim * (config.n_kv_heads * head_size); // wv
    total_weights += (size_t)config.n_layers * (config.n_heads * head_size) * config.dim;  // wo
    total_weights += (size_t)config.n_layers * config.dim;    // rms_ffn_weight
    ...
    // Read weights into all_weights (CORRECTED read sizes)
    // token_embedding_table
    file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)config.vocab_size * config.dim * sizeof(float));
    offset += (size_t)config.vocab_size * config.dim;

    // rms_att_weight
    file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)config.n_layers * config.dim * sizeof(float));
    offset += (size_t)config.n_layers * config.dim;

    // wq, wk, wv, wo per layer
    for (int l = 0; l < config.n_layers; ++l) {
        file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)config.dim * (config.n_heads * head_size) * sizeof(float));  // wq
        offset += (size_t)config.dim * (config.n_heads * head_size);
        file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)config.dim * (config.n_kv_heads * head_size) * sizeof(float));  // wk
        offset += (size_t)config.dim * (config.n_kv_heads * head_size);
        file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)config.dim * (config.n_kv_heads * head_size) * sizeof(float));  // wv
        offset += (size_t)config.dim * (config.n_kv_heads * head_size);
        file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)(config.n_heads * head_size) * config.dim * sizeof(float));  // wo
        offset += (size_t)(config.n_heads * head_size) * config.dim;
    }
    ...
    // w1, w2, w3 per layer
    for (int l = 0; l < config.n_layers; ++l) {
        file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)config.hidden_dim * config.dim * sizeof(float));  // w1
        offset += (size_t)config.hidden_dim * config.dim;
        file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)config.dim * config.hidden_dim * sizeof(float));  // w2
        offset += (size_t)config.dim * config.hidden_dim;
        file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)config.hidden_dim * config.dim * sizeof(float));  // w3
        offset += (size_t)config.hidden_dim * config.dim;
    }

    // rms_final_weight
    file.read(reinterpret_cast<char*>(&all_weights[offset]), (size_t)config.dim * sizeof(float));
    offset += (size_t)config.dim;
   ...
   // ...existing code...
    ...
    key_cache.resize((size_t)LAYERS * MAX_SEQ_LEN * DIM, 0.0f);
    value_cache.resize((size_t)LAYERS * MAX_SEQ_LEN * DIM, 0.0f);

    // 3. Call DUT
    std::cout << "Thực thi llama_layer..." << std::endl;
    llama_layer(token_embed.data(), output_logits.data(), all_weights.data(),
