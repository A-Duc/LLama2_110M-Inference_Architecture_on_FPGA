...
#pragma HLS INTERFACE m_axi port=sin_table offset=slave bundle=gmem16
    ...
    // Optimize KV cache for URAM utilization and bandwidth
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    // Optimized buffers for 12-layer LLaMA with enhanced partitioning
    static float hidden_state[kDim];
    static float attn_output[kDim];
    static float ffn_output[kDim];
    static float temp_buffer[kDim];
    
    // Enhanced array partitioning for better performance in 12-layer processing
#pragma HLS ARRAY_PARTITION variable=hidden_state cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=attn_output cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=ffn_output cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=temp_buffer cyclic factor=8 dim=1
    
    // Apply BRAM mapping for better resource utilization
#pragma HLS RESOURCE variable=hidden_state core=XPM_MEMORY uram=false
#pragma HLS RESOURCE variable=attn_output core=XPM_MEMORY uram=false
#pragma HLS RESOURCE variable=ffn_output core=XPM_MEMORY uram=false
...
#pragma HLS LOOP_TRIPCOUNT min=768 max=768
    ...
    // Process through all transformer layers - FULL 12 LAYERS FOR PRODUCTION
    transformer_layers: for (int layer = 0; layer < kNumLayers; layer++) {  // Full 12 layers
...
#pragma HLS ALLOCATION instances=matmul_kernel limit=2 function
    
    // Optimized buffers with URAM pragma for large arrays
    static float normed_input[kDim];
    static float q[kDim], k[kDim], v[kDim];
    static float q_rope[kDim], k_rope[kDim];
    
    // Enhanced array partitioning for better pipeline performance
...
#pragma HLS ARRAY_PARTITION variable=k cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=v cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=q_rope cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=k_rope cyclic factor=8 dim=1
    
    // STAGE 1: Pre-normalization with optimized pipeline
    rmsnorm_kernel(input, normed_input, norm_weights);
    
    // STAGE 2: Parallel Q,K,V projections using dataflow
#pragma HLS DATAFLOW
    {
    ...
    // STAGE 3: RoPE rotation with improved timing
    rope_kernel(q, k, q_rope, k_rope, cos_vals, sin_vals);
    
    // STAGE 4: Update KV cache with URAM optimization
    update_kv_cache_kernel(k_cache, v_cache, k_rope, v, position);
    
    // STAGE 5: Compute attention with optimized cache access
    static float attn_output[kDim];
#pragma HLS ARRAY_PARTITION variable=attn_output cyclic factor=8 dim=1
    
    compute_attention_with_cache_kernel(q_rope, k_cache, v_cache, attn_output, position + 1);
    
    // STAGE 6: Output projection with final optimization
...
//#pragma HLS PIPELINE II=1  // Remove pipeline - causes issues with variable trip count
...
#pragma HLS ALLOCATION instances=matmul_kernel limit=2 function
    ...
    // Enhanced partitioning for better performance with 12 layers
#pragma HLS ARRAY_PARTITION variable=normed_input cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=w1_out cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=w3_out cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=silu_out cyclic factor=8 dim=1
#pragma HLS ARRAY_PARTITION variable=gated_out cyclic factor=8 dim=1
    
    // 1. Pre-normalization with optimized timing
    rmsnorm_kernel(input, normed_input, norm_weights);
    
    // 2. Parallel W1 and W3 projections using dataflow
#pragma HLS DATAFLOW
    {
    ...
    // 3. SiLU activation on W1 output with pipeline optimization
