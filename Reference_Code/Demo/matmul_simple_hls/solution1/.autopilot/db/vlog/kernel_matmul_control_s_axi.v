...
// ==============================================================
`timescale 1ns/1ps
module kernel_matmul_control_s_axi
#(parameter
    ...
    output wire [63:0]                   i_mat,
    output wire [63:0]                   o_vec,
    output wire [31:0]                   vec_size,
...
localparam
    ...
    ADDR_I_MAT_DATA_0    = 7'h1c,
    ADDR_I_MAT_DATA_1    = 7'h20,
    ADDR_I_MAT_CTRL      = 7'h24,
    ADDR_O_VEC_DATA_0    = 7'h28,
    ADDR_O_VEC_DATA_1    = 7'h2c,
    ADDR_O_VEC_CTRL      = 7'h30,
    ADDR_VEC_SIZE_DATA_0 = 7'h34,
    ADDR_VEC_SIZE_CTRL   = 7'h38,
    ADDR_COL_SIZE_DATA_0 = 7'h3c,
    ADDR_COL_SIZE_CTRL   = 7'h40,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    WRRESET              = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_i_mat = 'b0;
    reg  [63:0]                   int_o_vec = 'b0;
    reg  [31:0]                   int_vec_size = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_I_MAT_DATA_0: begin
                    rdata <= int_i_mat[31:0];
                end
                ADDR_I_MAT_DATA_1: begin
                    rdata <= int_i_mat[63:32];
                end
                ...
                ADDR_VEC_SIZE_DATA_0: begin
                    rdata <= int_vec_size[31:0];
                end
...
assign i_mat             = int_i_mat;
assign o_vec             = int_o_vec;
assign vec_size          = int_vec_size;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_I_MAT_DATA_0)
            int_i_mat[31:0] <= (WDATA[31:0] & wmask) | (int_i_mat[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_I_MAT_DATA_1)
            int_i_mat[63:32] <= (WDATA[31:0] & wmask) | (int_i_mat[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_VEC_SIZE_DATA_0)
            int_vec_size[31:0] <= (WDATA[31:0] & wmask) | (int_vec_size[31:0] & ~wmask);
    end
end
