...
extern "C" {
__attribute__((sdx_kernel("kernel_matmul", 0))) void kernel_matmul(float* i_vec, float* i_mat, float* o_vec, int vec_size,
                   int col_size) {
#line 31 "D:/College/Project/LLma2_110M/Source_Code/run_matmul_simple.tcl"
#pragma HLSDIRECTIVE TOP name=kernel_matmul
# 5 "kernel_matmul.cpp"
...
#pragma HLS INTERFACE s_axilite port = return bundle = control
 ...
 float vec_local[768];
  ...
  VITIS_LOOP_17_1: for (int i = 0; i < vec_size && i < 768; i++) {
#pragma HLS PIPELINE II=1
 vec_local[i] = i_vec[i];
  }


  VITIS_LOOP_23_2: for (int i = 0; i < col_size && i < 2048; i++) {
#pragma HLS LOOP_TRIPCOUNT min=768 max=2048
 float sum = 0;
    VITIS_LOOP_26_3: for (int j = 0; j < vec_size && j < 768; j++) {
#pragma HLS PIPELINE II=1
