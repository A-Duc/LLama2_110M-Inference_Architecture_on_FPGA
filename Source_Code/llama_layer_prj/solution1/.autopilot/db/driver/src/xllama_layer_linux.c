...
typedef struct {
    ...
    XLlama_layer_uio_map maps[ MAX_UIO_MAPS ];
} XLlama_layer_uio_info;

/***************** Variable Definitions **************************************/
static XLlama_layer_uio_info uio_info;
...
static int uio_info_read_name(XLlama_layer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLlama_layer_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLlama_layer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLlama_layer_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLlama_layer_Initialize(XLlama_layer *InstancePtr, const char* InstanceName) {
	XLlama_layer_uio_info *InfoPtr = &uio_info;
