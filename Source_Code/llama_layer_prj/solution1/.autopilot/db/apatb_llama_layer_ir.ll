; ModuleID = 'C:/NCKH/LLama2_110M-Inference_Architecture_on_FPGA/Source_Code/llama_layer_prj/solution1/.autopilot/db/a.g.ld.5.gdce.bc'
source_filename = "llvm-link"
target datalayout = "e-m:e-i64:64-i128:128-i256:256-i512:512-i1024:1024-i2048:2048-i4096:4096-n8:16:32:64-S128-v16:16-v24:32-v32:32-v48:64-v96:128-v192:256-v256:256-v512:512-v1024:1024"
target triple = "fpga64-xilinx-none"

; Function Attrs: noinline willreturn
define void @apatb_llama_layer_ir(float* noalias nocapture nonnull "maxi" %token_embed, float* noalias nocapture nonnull "maxi" %output_logits, float* noalias nonnull readonly "maxi" %all_weights, float* noalias nocapture nonnull "maxi" %key_cache, float* noalias nocapture nonnull "maxi" %value_cache, i32 %position) local_unnamed_addr #0 {
entry:
  %token_embed_copy = alloca float, align 512
  %output_logits_copy = alloca float, align 512
  %all_weights_copy = alloca float, align 512
  %key_cache_copy = alloca float, align 512
  %value_cache_copy = alloca float, align 512
  call fastcc void @copy_in(float* nonnull %token_embed, float* nonnull align 512 %token_embed_copy, float* nonnull %output_logits, float* nonnull align 512 %output_logits_copy, float* nonnull %all_weights, float* nonnull align 512 %all_weights_copy, float* nonnull %key_cache, float* nonnull align 512 %key_cache_copy, float* nonnull %value_cache, float* nonnull align 512 %value_cache_copy)
  call void @apatb_llama_layer_hw(float* %token_embed_copy, float* %output_logits_copy, float* %all_weights_copy, float* %key_cache_copy, float* %value_cache_copy, i32 %position)
  call void @copy_back(float* %token_embed, float* %token_embed_copy, float* %output_logits, float* %output_logits_copy, float* %all_weights, float* %all_weights_copy, float* %key_cache, float* %key_cache_copy, float* %value_cache, float* %value_cache_copy)
  ret void
}
...
declare void @apatb_llama_layer_hw(float*, float*, float*, float*, float*, i32)
...
declare void @llama_layer_hw_stub(float* noalias nocapture nonnull, float* noalias nocapture nonnull, float* noalias nonnull readonly, float* noalias nocapture nonnull, float* noalias nocapture nonnull, i32)

define void @llama_layer_hw_stub_wrapper(float*, float*, float*, float*, float*, i32) #4 {
entry:
  call void @copy_out(float* null, float* %0, float* null, float* %1, float* null, float* %2, float* null, float* %3, float* null, float* %4)
  call void @llama_layer_hw_stub(float* %0, float* %1, float* %2, float* %3, float* %4, i32 %5)
