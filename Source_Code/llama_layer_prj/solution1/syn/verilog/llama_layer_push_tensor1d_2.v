// ==============================================================
// Generated by Vitis HLS v2025.1
// Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
// Copyright 2022-2025 Advanced Micro Devices, Inc. All Rights Reserved.
// ==============================================================

`timescale 1 ns / 1 ps 

module llama_layer_push_tensor1d_2 (
        ap_clk,
        ap_rst,
        ap_start,
        ap_done,
        ap_idle,
        ap_ready,
        x_strm_din,
        x_strm_full_n,
        x_strm_write,
        ffn_input_address0,
        ffn_input_ce0,
        ffn_input_q0,
        ffn_input_44_address0,
        ffn_input_44_ce0,
        ffn_input_44_q0,
        ffn_input_45_address0,
        ffn_input_45_ce0,
        ffn_input_45_q0,
        ffn_input_46_address0,
        ffn_input_46_ce0,
        ffn_input_46_q0,
        ffn_input_47_address0,
        ffn_input_47_ce0,
        ffn_input_47_q0,
        ffn_input_48_address0,
        ffn_input_48_ce0,
        ffn_input_48_q0,
        ffn_input_49_address0,
        ffn_input_49_ce0,
        ffn_input_49_q0,
        ffn_input_50_address0,
        ffn_input_50_ce0,
        ffn_input_50_q0
);

parameter    ap_ST_fsm_pp0_stage0 = 1'd1;

input   ap_clk;
input   ap_rst;
input   ap_start;
output   ap_done;
output   ap_idle;
output   ap_ready;
output  [31:0] x_strm_din;
input   x_strm_full_n;
output   x_strm_write;
output  [6:0] ffn_input_address0;
output   ffn_input_ce0;
input  [31:0] ffn_input_q0;
output  [6:0] ffn_input_44_address0;
output   ffn_input_44_ce0;
input  [31:0] ffn_input_44_q0;
output  [6:0] ffn_input_45_address0;
output   ffn_input_45_ce0;
input  [31:0] ffn_input_45_q0;
output  [6:0] ffn_input_46_address0;
output   ffn_input_46_ce0;
input  [31:0] ffn_input_46_q0;
output  [6:0] ffn_input_47_address0;
output   ffn_input_47_ce0;
input  [31:0] ffn_input_47_q0;
output  [6:0] ffn_input_48_address0;
output   ffn_input_48_ce0;
input  [31:0] ffn_input_48_q0;
output  [6:0] ffn_input_49_address0;
output   ffn_input_49_ce0;
input  [31:0] ffn_input_49_q0;
output  [6:0] ffn_input_50_address0;
output   ffn_input_50_ce0;
input  [31:0] ffn_input_50_q0;

reg ap_idle;

(* fsm_encoding = "none" *) reg   [0:0] ap_CS_fsm;
wire    ap_CS_fsm_pp0_stage0;
wire    ap_enable_reg_pp0_iter0;
reg    ap_enable_reg_pp0_iter1;
reg    ap_idle_pp0;
reg    ap_block_state2_pp0_stage0_iter1;
reg    ap_block_pp0_stage0_subdone;
wire   [0:0] icmp_ln12_fu_205_p2;
reg    ap_condition_exit_pp0_iter0_stage0;
wire    ap_loop_exit_ready;
reg    ap_ready_int;
reg    x_strm_blk_n;
wire    ap_block_pp0_stage0;
wire   [2:0] trunc_ln12_fu_211_p1;
reg   [2:0] trunc_ln12_reg_332;
reg    ap_block_pp0_stage0_11001;
wire   [63:0] zext_ln12_fu_225_p1;
reg   [9:0] i_fu_76;
wire   [9:0] add_ln12_fu_199_p2;
wire    ap_loop_init;
reg   [9:0] ap_sig_allocacmp_i_1;
wire   [31:0] bitcast_ln14_fu_313_p1;
reg    ap_block_pp0_stage0_01001;
reg    x_strm_write_local;
reg    ffn_input_ce0_local;
reg    ffn_input_44_ce0_local;
reg    ffn_input_45_ce0_local;
reg    ffn_input_46_ce0_local;
reg    ffn_input_47_ce0_local;
reg    ffn_input_48_ce0_local;
reg    ffn_input_49_ce0_local;
reg    ffn_input_50_ce0_local;
wire   [6:0] lshr_ln_fu_215_p4;
wire   [31:0] tmp_fu_274_p17;
wire   [31:0] tmp_fu_274_p19;
reg    ap_done_reg;
wire    ap_continue_int;
reg    ap_done_int;
reg   [0:0] ap_NS_fsm;
wire    ap_enable_pp0;
wire    ap_start_int;
wire    ap_ready_sig;
wire    ap_done_sig;
wire   [2:0] tmp_fu_274_p1;
wire   [2:0] tmp_fu_274_p3;
wire   [2:0] tmp_fu_274_p5;
wire   [2:0] tmp_fu_274_p7;
wire  signed [2:0] tmp_fu_274_p9;
wire  signed [2:0] tmp_fu_274_p11;
wire  signed [2:0] tmp_fu_274_p13;
wire  signed [2:0] tmp_fu_274_p15;
wire    ap_ce_reg;

// power-on initialization
initial begin
#0 ap_CS_fsm = 1'd1;
#0 ap_enable_reg_pp0_iter1 = 1'b0;
#0 i_fu_76 = 10'd0;
#0 ap_done_reg = 1'b0;
end

(* dissolve_hierarchy = "yes" *) llama_layer_sparsemux_17_3_32_1_1 #(
    .ID( 1 ),
    .NUM_STAGE( 1 ),
    .CASE0( 3'h0 ),
    .din0_WIDTH( 32 ),
    .CASE1( 3'h1 ),
    .din1_WIDTH( 32 ),
    .CASE2( 3'h2 ),
    .din2_WIDTH( 32 ),
    .CASE3( 3'h3 ),
    .din3_WIDTH( 32 ),
    .CASE4( 3'h4 ),
    .din4_WIDTH( 32 ),
    .CASE5( 3'h5 ),
    .din5_WIDTH( 32 ),
    .CASE6( 3'h6 ),
    .din6_WIDTH( 32 ),
    .CASE7( 3'h7 ),
    .din7_WIDTH( 32 ),
    .def_WIDTH( 32 ),
    .sel_WIDTH( 3 ),
    .dout_WIDTH( 32 ))
sparsemux_17_3_32_1_1_U1443(
    .din0(ffn_input_q0),
    .din1(ffn_input_44_q0),
    .din2(ffn_input_45_q0),
    .din3(ffn_input_46_q0),
    .din4(ffn_input_47_q0),
    .din5(ffn_input_48_q0),
    .din6(ffn_input_49_q0),
    .din7(ffn_input_50_q0),
    .def(tmp_fu_274_p17),
    .sel(trunc_ln12_reg_332),
    .dout(tmp_fu_274_p19)
);

llama_layer_flow_control_loop_pipe_sequential_init flow_control_loop_pipe_sequential_init_U(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(ap_start),
    .ap_ready(ap_ready_sig),
    .ap_done(ap_done_sig),
    .ap_start_int(ap_start_int),
    .ap_loop_init(ap_loop_init),
    .ap_ready_int(ap_ready_int),
    .ap_loop_exit_ready(ap_condition_exit_pp0_iter0_stage0),
    .ap_loop_exit_done(ap_done_int),
    .ap_continue_int(ap_continue_int),
    .ap_done_int(ap_done_int)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_CS_fsm <= ap_ST_fsm_pp0_stage0;
    end else begin
        ap_CS_fsm <= ap_NS_fsm;
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_done_reg <= 1'b0;
    end else begin
        if ((ap_continue_int == 1'b1)) begin
            ap_done_reg <= 1'b0;
        end else if (((ap_loop_exit_ready == 1'b1) & (1'b0 == ap_block_pp0_stage0_subdone) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
            ap_done_reg <= 1'b1;
        end
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_enable_reg_pp0_iter1 <= 1'b0;
    end else begin
        if ((1'b1 == ap_condition_exit_pp0_iter0_stage0)) begin
            ap_enable_reg_pp0_iter1 <= 1'b0;
        end else if (((1'b0 == ap_block_pp0_stage0_subdone) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
            ap_enable_reg_pp0_iter1 <= ap_start_int;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if (((icmp_ln12_fu_205_p2 == 1'd0) & (ap_enable_reg_pp0_iter0 == 1'b1))) begin
            i_fu_76 <= add_ln12_fu_199_p2;
        end else if ((ap_loop_init == 1'b1)) begin
            i_fu_76 <= 10'd0;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        trunc_ln12_reg_332 <= trunc_ln12_fu_211_p1;
    end
end

always @ (*) begin
    if (((icmp_ln12_fu_205_p2 == 1'd1) & (1'b0 == ap_block_pp0_stage0_subdone) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_condition_exit_pp0_iter0_stage0 = 1'b1;
    end else begin
        ap_condition_exit_pp0_iter0_stage0 = 1'b0;
    end
end

always @ (*) begin
    if (((ap_loop_exit_ready == 1'b1) & (1'b0 == ap_block_pp0_stage0_subdone) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_done_int = 1'b1;
    end else begin
        ap_done_int = ap_done_reg;
    end
end

always @ (*) begin
    if (((ap_start_int == 1'b0) & (ap_idle_pp0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_idle = 1'b1;
    end else begin
        ap_idle = 1'b0;
    end
end

always @ (*) begin
    if (((ap_enable_reg_pp0_iter1 == 1'b0) & (ap_enable_reg_pp0_iter0 == 1'b0))) begin
        ap_idle_pp0 = 1'b1;
    end else begin
        ap_idle_pp0 = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0_subdone) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_ready_int = 1'b1;
    end else begin
        ap_ready_int = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1))) begin
        ap_sig_allocacmp_i_1 = 10'd0;
    end else begin
        ap_sig_allocacmp_i_1 = i_fu_76;
    end
end

always @ (*) begin
    if (((icmp_ln12_fu_205_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ffn_input_44_ce0_local = 1'b1;
    end else begin
        ffn_input_44_ce0_local = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln12_fu_205_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ffn_input_45_ce0_local = 1'b1;
    end else begin
        ffn_input_45_ce0_local = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln12_fu_205_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ffn_input_46_ce0_local = 1'b1;
    end else begin
        ffn_input_46_ce0_local = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln12_fu_205_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ffn_input_47_ce0_local = 1'b1;
    end else begin
        ffn_input_47_ce0_local = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln12_fu_205_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ffn_input_48_ce0_local = 1'b1;
    end else begin
        ffn_input_48_ce0_local = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln12_fu_205_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ffn_input_49_ce0_local = 1'b1;
    end else begin
        ffn_input_49_ce0_local = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln12_fu_205_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ffn_input_50_ce0_local = 1'b1;
    end else begin
        ffn_input_50_ce0_local = 1'b0;
    end
end

always @ (*) begin
    if (((icmp_ln12_fu_205_p2 == 1'd0) & (1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ffn_input_ce0_local = 1'b1;
    end else begin
        ffn_input_ce0_local = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        x_strm_blk_n = x_strm_full_n;
    end else begin
        x_strm_blk_n = 1'b1;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        x_strm_write_local = 1'b1;
    end else begin
        x_strm_write_local = 1'b0;
    end
end

always @ (*) begin
    case (ap_CS_fsm)
        ap_ST_fsm_pp0_stage0 : begin
            ap_NS_fsm = ap_ST_fsm_pp0_stage0;
        end
        default : begin
            ap_NS_fsm = 'bx;
        end
    endcase
end

assign add_ln12_fu_199_p2 = (ap_sig_allocacmp_i_1 + 10'd1);

assign ap_CS_fsm_pp0_stage0 = ap_CS_fsm[32'd0];

assign ap_block_pp0_stage0 = ~(1'b1 == 1'b1);

always @ (*) begin
    ap_block_pp0_stage0_01001 = ((ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_block_state2_pp0_stage0_iter1));
end

always @ (*) begin
    ap_block_pp0_stage0_11001 = ((ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_block_state2_pp0_stage0_iter1));
end

always @ (*) begin
    ap_block_pp0_stage0_subdone = ((ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_block_state2_pp0_stage0_iter1));
end

always @ (*) begin
    ap_block_state2_pp0_stage0_iter1 = (x_strm_full_n == 1'b0);
end

assign ap_done = ap_done_sig;

assign ap_enable_pp0 = (ap_idle_pp0 ^ 1'b1);

assign ap_enable_reg_pp0_iter0 = ap_start_int;

assign ap_loop_exit_ready = ap_condition_exit_pp0_iter0_stage0;

assign ap_ready = ap_ready_sig;

assign bitcast_ln14_fu_313_p1 = tmp_fu_274_p19;

assign ffn_input_44_address0 = zext_ln12_fu_225_p1;

assign ffn_input_44_ce0 = ffn_input_44_ce0_local;

assign ffn_input_45_address0 = zext_ln12_fu_225_p1;

assign ffn_input_45_ce0 = ffn_input_45_ce0_local;

assign ffn_input_46_address0 = zext_ln12_fu_225_p1;

assign ffn_input_46_ce0 = ffn_input_46_ce0_local;

assign ffn_input_47_address0 = zext_ln12_fu_225_p1;

assign ffn_input_47_ce0 = ffn_input_47_ce0_local;

assign ffn_input_48_address0 = zext_ln12_fu_225_p1;

assign ffn_input_48_ce0 = ffn_input_48_ce0_local;

assign ffn_input_49_address0 = zext_ln12_fu_225_p1;

assign ffn_input_49_ce0 = ffn_input_49_ce0_local;

assign ffn_input_50_address0 = zext_ln12_fu_225_p1;

assign ffn_input_50_ce0 = ffn_input_50_ce0_local;

assign ffn_input_address0 = zext_ln12_fu_225_p1;

assign ffn_input_ce0 = ffn_input_ce0_local;

assign icmp_ln12_fu_205_p2 = ((ap_sig_allocacmp_i_1 == 10'd768) ? 1'b1 : 1'b0);

assign lshr_ln_fu_215_p4 = {{ap_sig_allocacmp_i_1[9:3]}};

assign tmp_fu_274_p17 = 'bx;

assign trunc_ln12_fu_211_p1 = ap_sig_allocacmp_i_1[2:0];

assign x_strm_din = bitcast_ln14_fu_313_p1;

assign x_strm_write = x_strm_write_local;

assign zext_ln12_fu_225_p1 = lshr_ln_fu_215_p4;

endmodule //llama_layer_push_tensor1d_2
