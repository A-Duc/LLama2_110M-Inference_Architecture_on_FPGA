    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_CURRENT_TOKEN_DATA_0: begin
                    rdata <= int_current_token[31:0];
                end
                ADDR_CURRENT_TOKEN_DATA_1: begin
                    rdata <= int_current_token[63:32];
                end
...
// int_current_token[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_current_token[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CURRENT_TOKEN_DATA_0)
            int_current_token[31:0] <= (WDATA[31:0] & wmask) | (int_current_token[31:0] & ~wmask);
    end
end

// int_current_token[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_current_token[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CURRENT_TOKEN_DATA_1)
            int_current_token[63:32] <= (WDATA[31:0] & wmask) | (int_current_token[63:32] & ~wmask);
    end
end
